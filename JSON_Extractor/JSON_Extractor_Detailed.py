...
def get_product_data(url: str = 'https://www.nintendo.com/us/store/products/sword-of-the-vagrant-switch/') -> None:
    ...
    # Получение более расширенных данных
    script_v2_tag = soup.find('script', id='__NEXT_DATA__', type='application/json')

    if script_v2_tag:

        json_text = script_v2_tag.string
        ...
        if not os.path.exists(f'json_dumps/{timestamp_str_short}/detailed'):
            os.makedirs(f'json_dumps/{timestamp_str_short}/detailed')

        # Сохраняю результаты парсинга в json-файл
        with open(f'json_dumps/{timestamp_str_short}/detailed/{name}__output_{timestamp_str}.json', 'w', encoding='utf-8') as file:
